    ...
    where TValue : IComparable<TValue>, IEquatable<TValue>
{
    ...
    public static TEnum FromName(string name)
        => Options.Value.FirstOrDefault(x => x.Name.Equals(name, StringComparison.OrdinalIgnoreCase))
        ?? throw new ArgumentException($"No {typeof(TEnum).Name} with name '{name}' found.");
    
    public virtual bool TryFromName(string name, out TEnum? result)
    {
        
        result = List.FirstOrDefault(x => x.Name.Equals(name, StringComparison.OrdinalIgnoreCase));
        return result != null;
    }
    ...
    public virtual bool TryFromValue(TValue value, out TEnum? result)
    {
        result = Options.Value.FirstOrDefault(x => x.Value.Equals(value));
